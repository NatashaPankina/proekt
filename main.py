...
def start_page():
    map_request = "http://static-maps.yandex.ru/1.x/?ll=85.530887,60.703118&spn=34.077777,34.077777&l=map"
    response = requests.get(map_request)
    with open("static/img/map.png", "wb") as file:
        if 'Error 502' not in str(response.content):
            file.write(response.content)
    return render_template('first.html', name="Светик")
...
def konvert_output():
    ...
    if request.method == 'POST':
        try:
            ...
            print(e)
            return render_template('err.html', message="Проверьте корректность файла")
...
def edit(id):
    if request.method == "GET":
        ...
        if res:
            ...
            return render_template('err.html', message="Проверьте корректность запроса")
